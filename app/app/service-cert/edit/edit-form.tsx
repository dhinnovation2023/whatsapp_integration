...
const EditServiceCustomerForm = ({ data, brands }: {
    ...
    const fieldsData: InputGroupDataInterface[] = [
        ...
        },
        {
            ...
            required: true,
            type: "date",
            value: formData.dateOfService instanceof Date ? `${formData.dateOfService.getFullYear()}-${(formData.dateOfService.getMonth() + 1).toString().padStart(2, '0')}-${formData.dateOfService.getDate().toString().padStart(2, '0')}` : undefined,
        },
        {
            label: "Service count",
            name: "nthService",
